...
if (isset($_POST['add'])) {
    ...
    if (!empty($product_name) && !empty($category) && $price >= 0 && $quantity >= 0) {
        $stmt = $conn->prepare("INSERT INTO products (product_name, category, description, price, quantity) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssdi", $product_name, $category, $description, $price, $quantity);
        $stmt->execute();
    } else {
...
if (isset($_POST['edit'])) {
    ...
    if (!empty($product_name) && !empty($category) && $price >= 0 && $quantity >= 0) {
        $stmt = $conn->prepare("UPDATE products SET product_name=?, category=?, description=?, price=?, quantity=? WHERE id=?");
        $stmt->bind_param("sssdiI", $product_name, $category, $description, $price, $quantity, $id);
        $stmt->execute();
    } else {
